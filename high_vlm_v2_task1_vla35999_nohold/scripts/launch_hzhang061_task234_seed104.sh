...
if [[ ! "${TASK_ID}" =~ ^(2|3|4)$ ]]; then
  echo "TASK_ID must be 2, 3, or 4; got ${TASK_ID}" >&2
  exit 1
fi
...
RUN_ID="${RUN_ID:-task${TASK_ID}_seed104_highvlmv2_nohold_$(date +%Y%m%d_%H%M%S)}"
...
cp "${SCRIPT_DIR}/run_task234_seed104_nohold_inside_allocation.sh" \
  "${FROZEN_SCRIPT_DIR}/run_task234_seed104_nohold_inside_allocation.sh"
cp "${EXP_ROOT}/extensions/eval_task234_physical.py" \
  "${FROZEN_SCRIPT_DIR}/eval_task234_physical.py"
chmod 755 "${FROZEN_SCRIPT_DIR}"/*.sh "${FROZEN_SCRIPT_DIR}/eval_task234_physical.py"
sha256sum \
...
FROZEN_RUNNER="${FROZEN_SCRIPT_DIR}/run_task234_seed104_nohold_inside_allocation.sh"
FROZEN_EVALUATOR="${FROZEN_SCRIPT_DIR}/eval_task234_physical.py"
...
run_request() {
  ...
  local time_limit="$5"
  shift 5
  local request_log="${PROBE_DIR}/${label}_acd_u.log"

  echo "request label=${label} partition=acd_u gpus=${gpus} cpus=${cpus} mem=${mem}"
  srun \
    --partition=acd_u \
    --nodes=1 \
    --ntasks=1 \
  ...
  echo "request passed label=${label} partition=acd_u"
}
...
run_request \
  ...
  env TASK_ID="${TASK_ID}" RUN_ID="${RUN_ID}" VLA_CHECKPOINT="${VLA_CHECKPOINT}" \
    VLM_ADAPTER="${VLM_ADAPTER}" VLM_BASE="${VLM_BASE}" \
    TARGET_LIBERO_PATH="${TARGET_LIBERO_PATH}" EVAL_PY="${EVAL_PY}" \
    EVALUATOR="${FROZEN_EVALUATOR}" HV2_EXP_ROOT="${EXP_ROOT}" \
    EXP_ROOT="${EXP_ROOT}" AUDIT_SCRIPT_DIR="${SCRIPT_DIR}" "${FROZEN_RUNNER}"

echo "launcher complete task=${TASK_ID} run_id=${RUN_ID}"
