...
- Recurrence lesson: the repository previously preserved diagnosis and invalid-run handling, but no verified fix. The required evidence and 300-step smoke gate are now frozen in `docs/NATIVE_RENDER_ABORT_LESSON.md`; multi-task expansion is blocked until that gate passes.
...
- Frozen composition: `TWO_CALL_EXTENSION_PATH` is mandatory in the formal runner and points at the run-local hashed copy of `eval_two_call_prompt_commit.py`; test coverage verifies loading from an arbitrary frozen path.
- Formal runner: `scripts/run_all26_one_ep_inside_allocation.sh` validates Task1-26, submitting Unix user, two visible GPUs, latest snapshot, scorer/BDDL hashes, checkpoint-local norm, and both frozen adapters before model launch.
