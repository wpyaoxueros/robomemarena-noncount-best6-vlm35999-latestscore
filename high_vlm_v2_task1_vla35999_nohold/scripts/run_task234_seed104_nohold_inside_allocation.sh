...
EVALUATOR="${EVALUATOR:-${EXP_ROOT}/extensions/eval_task234_physical.py}"
OFFICIAL_SCORER="${PACK_ROOT}/official_snapshot/evaluation_benchmark/scripts/task2_26_reference_stage.py"
OFFICIAL_SCORER_SHA256="0ab5e19cb7b90844b86fe04a76facc0364af55f1e841c4754aa675404a318538"
OFFICIAL_BDDL_DIR="${PACK_ROOT}/official_snapshot/evaluation_benchmark/bddl"
...
payload = {
    ...
    "task": ${TASK_ID},
    "seed": 104,
    "control_contract": "synchronous high-vlm-v2 prompt -> VLA35999 actions; no hold/release/anchor/oracle/GT replay",
    "vla": {
        "config": "${VLA_CONFIG}",
    ...
    "official_scoring": {
        ...
        "task4_final_close_optional": ${TASK_ID} == 4,
    },
...
printf '%q ' env CUDA_VISIBLE_DEVICES=1 HV2_EXP_ROOT="${EXP_ROOT}" "${eval_cmd[@]}" > "${COMMAND_FILE}"
printf '\n' >> "${COMMAND_FILE}"

set +e
CUDA_VISIBLE_DEVICES=1 HV2_EXP_ROOT="${EXP_ROOT}" PYTHONFAULTHANDLER=1 \
...
echo "TASK${TASK_ID}_EVAL_COMPLETE run_root=${RUN_ROOT}"
