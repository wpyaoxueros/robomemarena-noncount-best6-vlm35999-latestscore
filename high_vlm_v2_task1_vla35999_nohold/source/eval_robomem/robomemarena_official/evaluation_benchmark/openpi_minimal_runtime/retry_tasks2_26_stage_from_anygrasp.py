...
# Kept as a compatibility API for the official reference evaluator. Upstream
# commit 514ecdf still calls these helpers after removing their definitions.
COUNTING_POUR_TASK_TARGETS = {
    6: ("tomato_sauce_1", "body", "cookies_1"),
    7: ("tomato_sauce_1", "site", "frypan_1_default_site"),
    8: ("tomato_sauce_1", "body", "chocolate_pudding_1"),
    9: ("tomato_sauce_1", "body", "butter_1"),
    10: ("wine_bottle_1", "site", "white_yellow_mug_1_default_site"),
    15: ("milk_1", "body", "butter_1"),
    16: ("milk_1", "site", "red_coffee_mug_1_default_site"),
    22: ("tomato_sauce_1", "body", "cookies_1"),
}


def _is_counting_pour_task(task_id: int) -> bool:
    return int(task_id) in COUNTING_POUR_TASK_TARGETS
...
def _extra_pour_check(task_id: int) -> Callable[[Any, dict[str, Any], int], bool] | None:
    target = COUNTING_POUR_TASK_TARGETS.get(int(task_id))
    if target is None:
        return None
